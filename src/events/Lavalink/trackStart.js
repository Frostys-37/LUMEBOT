...
module.exports = async (client, player, track, payload) => {
  ...
  if (data) {
    ...
    if (!textChannel) {
      try {
        ...
        channel.send("Usa el comando /setup para poder usar mis funciones");
        textChannel = channel;
      }
    }
  ...
  const thing = new EmbedBuilder()
    .setDescription(
      `${emojiplay} **Empezando a reproducir**\n [${track?.title ?? queue.title}](${
        track?.uri ?? queue.uri
  ...
  const collector = m.createMessageComponentCollector({
    filter: (b) => {
      if (
        ...
        b.reply({
          content: `No estas conectad@ en: ${b.guild.members.me.voice.channel.toString()} para usar estos botones.`,
          ephemeral: true,
        });
        return false;
      }
      ...
      i.editReply({
        embeds: [
          embed
            ...
            .setDescription(
              `${volumeEmoji} El volumen actual es: **${amount}**`
            ),
        ],
      }).then((msg) => {
        setTimeout(() => {
      ...
      i.editReply({
        embeds: [
          embed
            ...
            .setDescription(`${emojistop} Musica parada.`),
        ],
      }).then((msg) => {
        setTimeout(() => {
          msg.delete();
          ...
          .editReply({
            embeds: [
              embed
                ...
                .setDescription(
                  `No se puede subir más el volumen del reproductor.`
                ),
            ],
          })
          .then((msg) => {
