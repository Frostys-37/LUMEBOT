...
module.exports = {
    ...
    run: async (client, interaction,) => {
        ...
        if (!interaction.guild.members.me.permissions.has(PermissionsBitField.resolve(['Speak', 'Connect']))) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(client.embedColor).setDescription(`No tengo permisos suficientes, necesito: \`CONNECT\` o \`SPEAK\`.`)] });
        const { channel } = interaction.member.voice;
        if (!interaction.guild.members.cache.get(client.user.id).permissionsIn(channel).has(PermissionsBitField.resolve(['Speak', 'Connect']))) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(client.embedColor).setDescription(`No tengo permisos para conectarme al canal de voz, necesito: \`CONNECT\` o \`SPEAK\`.`)] });
        ...
        switch (s.loadType) {
            case "TRACK_LOADED":
                ...
                const embed = new EmbedBuilder()
                    .setDescription(`${emojiplaylist} **Agregada a la lista** - [${s.tracks[0].title}](${s.tracks[0].uri}) \`${convertTime(s.tracks[0].duration, true)}\` • ${s.tracks[0].requester}`)
            ...
            case "SEARCH_RESULT":
                ...
                const searched = new EmbedBuilder()
                    .setTitle("Selecciona la plista que quieras.")
                    .setColor(client.embedColor)
                ...
                collector.on("collect", async (b) => {
                    ...
                    if (b.customId === "s_one") {
                        ...
                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Añadida a la lista.** - [${s.tracks[0].title}](${s.tracks[0].uri}) \`${convertTime(s.tracks[0].duration, true)}\` • ${interaction.member.user}`)] })
                    } else if (b.customId === "s_two") {
                        player.queue.add(s.tracks[1]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Añadida a la lista.** - [${s.tracks[1].title}](${s.tracks[1].uri}) \`${convertTime(s.tracks[1].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_three") {
                        player.queue.add(s.tracks[2]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Añadida a la lista.** - [${s.tracks[2].title}](${s.tracks[2].uri}) \`${convertTime(s.tracks[2].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_four") {
                        player.queue.add(s.tracks[3]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Añadida a la lista.** - [${s.tracks[3].title}](${s.tracks[3].uri}) \`${convertTime(s.tracks[3].duration, true)}\` • ${interaction.member.user}`)] })

                    } else if (b.customId === "s_five") {
                        player.queue.add(s.tracks[4]);
                        if (player && player.state === "CONNECTED" && !player.playing && !player.paused && !player.queue.size) await player.play();

                        await interaction.editReply({ embeds: [search.setDescription(`${emojiplaylist} **Añadida a la lista.** - [${s.tracks[4].title}](${s.tracks[4].uri}) \`${convertTime(s.tracks[4].duration, true)}\` • ${s.tracks[4].requester}`)] })
